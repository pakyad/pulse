const admin = require('firebase-admin');

const serviceAccount = {
  projectId: "codep-pulse",
  clientEmail: "firebase-adminsdk-fbsvc@codep-pulse.iam.gserviceaccount.com",
  privateKey: "-----BEGIN PRIVATE KEY-----\\\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n')
};

if (!admin.apps.length) admin.initializeApp({ 
  credential: admin.credential.cert(serviceAccount) 
});
const db = admin.firestore();

async function purgeMuhaiminReal() {
  const uid = 'KcpwVBvWDISy56Oy0bKi3CinEVo1';
  console.log('--- ATOMIC PURGE: Muhaimin (Kcpw...) ---');
  
  const orders = await db.collection('orders').where('buyer_id', '==', uid).get();
  const txs = await db.collection('transactions').where('buyer_id', '==', uid).get();
  
  const batch = db.batch();
  orders.docs.forEach(d => batch.delete(d.ref));
  txs.docs.forEach(d => batch.delete(d.ref));
  
  await batch.commit();
  console.log(`CLEARED: ${orders.size} orders and ${txs.size} legacy transactions.`);
  console.log('--- PURGE COMPLETE ---');
  process.exit(0);
}

purgeMuhaiminReal();
