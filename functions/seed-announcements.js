const admin = require('firebase-admin');

const privateKey = "-----BEGIN PRIVATE KEY-----\\\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n');

admin.initializeApp({
  credential: admin.credential.cert({
    projectId: "codep-pulse",
    clientEmail: "firebase-adminsdk-fbsvc@codep-pulse.iam.gserviceaccount.com",
    privateKey: privateKey
  })
});

const db = admin.firestore();

const announcements = [
  {
    tag: 'MARKETPLACE',
    headline: 'Trending: Student Tech',
    subline: 'Upgrade your workflow with the latest student-listed tech gear now in the hub.',
    imageUrl: 'https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&q=80&w=1200',
    color: '#0f172a',
    ctaPath: '/marketplace',
    created_at: admin.firestore.FieldValue.serverTimestamp(),
  },
  {
    tag: 'INSTITUTIONAL',
    headline: 'SE Club Hackathon',
    subline: 'Registration is now open for the annual campus-wide innovation sprint.',
    imageUrl: 'https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&q=80&w=1200',
    color: '#1e293b',
    ctaPath: '/pulse',
    created_at: admin.firestore.FieldValue.serverTimestamp(),
  },
  {
    tag: 'COMMUNITY',
    headline: 'Found: Blue Lanyard',
    subline: 'A set of keys with a blue lanyard was found at Block G. Claim at the security desk.',
    color: '#1e293b',
    ctaPath: '/home',
    created_at: admin.firestore.FieldValue.serverTimestamp(),
  },
  {
    tag: 'CAMPUS LIFE',
    headline: 'Hostel Services: Block K',
    subline: 'Scheduled water maintenance this Saturday. Please plan your schedule accordingly.',
    imageUrl: 'https://images.unsplash.com/photo-1555854811-822427f1cfba?auto=format&fit=crop&q=80&w=1200',
    color: '#334155',
    ctaPath: '/pulse',
    created_at: admin.firestore.FieldValue.serverTimestamp(),
  }
];




async function seed() {
  try {
    const col = db.collection('announcements');
    
    // Clear existing announcements first (Optional, but good for clean demo)
    const snapshot = await col.get();
    const batch = db.batch();
    snapshot.docs.forEach(doc => batch.delete(doc.ref));
    await batch.commit();
    console.log('Cleared existing announcements.');

    for (const ann of announcements) {
      await col.add(ann);
    }
    console.log('✅ Announcements seeded successfully.');
    process.exit(0);
  } catch (err) {
    console.error('❌ Seeding failed:', err);
    process.exit(1);
  }
}

seed();
