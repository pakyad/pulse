const admin = require('firebase-admin');

const privateKey = "-----BEGIN PRIVATE KEY-----\\\\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n');

admin.initializeApp({
  credential: admin.credential.cert({
    projectId: "codep-pulse",
    clientEmail: "firebase-adminsdk-fbsvc@codep-pulse.iam.gserviceaccount.com",
    privateKey: privateKey
  })
});

const db = admin.firestore();
const auth = admin.auth();

async function seed() {
  try {
    // 1. Create/Get User
    let user;
    try {
      user = await auth.getUserByEmail('SEclub@merchant.com');
      console.log('User already exists:', user.uid);
    } catch (e) {
      user = await auth.createUser({
        email: 'SEclub@merchant.com',
        password: 'Iyad123',
        displayName: 'SE Club (Software Engineering Society)'
      });
      console.log('Created user:', user.uid);
    }

    // 2. Set Roles
    await db.collection('users').doc(user.uid).set({
      full_name: 'SE Club (Software Engineering Society)',
      email: 'SEclub@merchant.com',
      role: 'CLUB',
      is_verified_merchant: true,
      matric_no: '51234567890',
      created_at: admin.firestore.FieldValue.serverTimestamp()
    }, { merge: true });

    // 3. Seed Listings
    const listings = [
      {
        title: "Arduino Uno Starter Kit",
        price: 250,
        stock_count: 5,
        image_url: "https://images.unsplash.com/photo-1553406830-ef2513450d76?auto=format&fit=crop&q=80&w=800",
        seller_id: user.uid,
        status: "active",
        category: "Tech",
        is_official: true,
        created_at: admin.firestore.FieldValue.serverTimestamp()
      },
      {
        title: "Raspberry Pi 4 Model B (4GB)",
        price: 350,
        stock_count: 3,
        image_url: "https://images.unsplash.com/photo-1563770660941-20978e870813?auto=format&fit=crop&q=80&w=800",
        seller_id: user.uid,
        status: "active",
        category: "Tech",
        is_official: true,
        created_at: admin.firestore.FieldValue.serverTimestamp()
      },
      {
        title: "Keychron V1 Mechanical Keyboard",
        price: 180,
        stock_count: 2,
        image_url: "https://images.unsplash.com/photo-1511467687858-23d96c32e4ae?auto=format&fit=crop&q=80&w=800",
        seller_id: user.uid,
        status: "active",
        category: "Tech",
        is_official: true,
        created_at: admin.firestore.FieldValue.serverTimestamp()
      },
      {
        title: "Official SE Society Lanyard",
        price: 15,
        stock_count: 50,
        image_url: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?auto=format&fit=crop&q=80&w=800",
        seller_id: user.uid,
        status: "active",
        category: "Apparel",
        is_official: true,
        created_at: admin.firestore.FieldValue.serverTimestamp()
      },
      {
        title: "Introduction to Algorithms (CLRS)",
        price: 120,
        stock_count: 5,
        image_url: "https://images.unsplash.com/photo-1532012197267-da84d127e765?auto=format&fit=crop&q=80&w=800",
        seller_id: user.uid,
        status: "active",
        category: "Stationery",
        is_official: true,
        created_at: admin.firestore.FieldValue.serverTimestamp()
      }
    ];

    for (const listing of listings) {
      await db.collection('items').add(listing);
    }

    console.log('Seeding successful for SE Club.');
    process.exit(0);
  } catch (err) {
    console.error('Seeding failed:', err);
    process.exit(1);
  }
}

seed();
