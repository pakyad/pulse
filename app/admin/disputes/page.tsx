    ...
    <>
      ...
      <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
        ...
        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
          <div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Dispute #{dispute.id.substring(0, 8).toUpperCase()}</p>
            <h2 className="text-[20px] font-black text-slate-900 tracking-tight">Review Case</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6">
          <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
            <div>
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Issue</p>
              <p className="text-[15px] font-bold text-slate-900">{dispute.reason || 'No reason provided'}</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Reported by</p>
                <p className="text-[13px] font-bold text-slate-900">{dispute.reporter_name || '—'}</p>
              </div>
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Filed</p>
        ...
        <div className="p-8 border-t border-slate-100 space-y-3">
          ...
          <button onClick={() => onResolve(dispute.id, 'RELEASE')} disabled={isProcessing}
            className="w-full h-13 bg-slate-900 text-white rounded-xl font-bold text-[13px] flex items-center justify-center gap-2 hover:bg-black transition-all active:scale-[0.98] disabled:opacity-50">
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }}
            className="fixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-full shadow-lg bg-[#2A5C50] text-white text-[13px] font-bold flex items-center gap-2">
      ...
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Conflict Resolution</p>
          <h1 className="text-[24px] font-black text-slate-900 tracking-tight">Disputes</h1>
        </div>
        <div className="px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl">
          <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest">{disputes.length} open</span>
        </div>
      </div>
            ...
            <button key={d.id} onClick={() => setSelected(d)}
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">#{d.id.substring(0,8).toUpperCase()}</span>
                    <span className="w-1 h-1 bg-slate-200 rounded-full" />
                    <span className="text-[10px] font-black text-red-500 uppercase">Awaiting Review</span>
                  </div>
              ...
              <div className="flex items-center gap-5">
                <div className="text-right">
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Reported by</p>
