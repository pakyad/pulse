...
export default function OrdersPage() {
  ...
  return (
    <main className="min-h-screen bg-white font-sans antialiased text-navy max-w-md mx-auto border-x border-[#F2F2F7] relative">
      ...
      {/* ── HEADER (Grab Inspired) ── */}
      <nav className="px-6 pt-12 pb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button 
            onClick={() => router.back()} 
            className="p-1 -ml-1 text-navy/30 hover:text-navy transition-all active:scale-90"
          >
            <ChevronLeft size={24} strokeWidth={2.5} />
          </button>
          <h1 className="text-[28px] font-bold text-black tracking-tight">Orders</h1>
        </div>
        
        <motion.button 
          whileTap={{ scale: 0.95 }}
          onClick={() => router.push('/me/orders/history')}
          className="h-9 px-4 bg-slate-50 rounded-full flex items-center gap-2 border border-[#F2F2F7] active:bg-slate-100 transition-colors"
        >
          <History size={16} className="text-navy" strokeWidth={2} />
          <span className="text-[13px] font-bold text-navy">History</span>
        </motion.button>
      </nav>
      ...
      <div className="px-6 pt-4">
        {ACTIVE_ORDERS.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center pt-20 text-center">
            ...
            <h2 className="text-[20px] font-bold text-black tracking-tight mb-2">Nothing's happening now</h2>
            <p className="text-[14px] text-slate-400 font-medium px-12 leading-relaxed">
