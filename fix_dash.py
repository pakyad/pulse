...
path = r'd:\FYP\Pulse\app\admin\dashboard\page.tsx'
with open(path, 'r', encoding='utf-8') as f:
    content = f.read()

content = content.replace("if (activeSubTab === 'RUNNER') return u.is_verified_runner;", "if (activeSubTab === 'RUNNER') return u.is_verified_runner || u.runner_status === 'pending';")
content = content.replace("if (activeSubTab === 'MERCHANT') return u.is_seller;", "if (activeSubTab === 'MERCHANT') return u.is_seller || u.merchant_status === 'pending';")
content = content.replace("if (activeSubTab === 'STUDENT') return !u.is_verified_runner && !u.is_seller;", "if (activeSubTab === 'STUDENT') return !u.is_verified_runner && !u.is_seller && u.runner_status !== 'pending' && u.merchant_status !== 'pending';")
