            ...
            {isSuccess ? (
              <div className="p-12 text-center space-y-6">
                 ...
                 <div className="space-y-2">
                    <h3 className="text-[17px] font-bold text-slate-900">Dispute Logged</h3>
                         ...
                         <div className="px-5 py-3 flex items-start justify-between">
                            ...
                            <div className="text-right">
                               {order?.items?.map((it: any, i: number) => (
                                  <p key={i} className="text-[11px] font-bold text-slate-900">{it.qty}x {it.title}</p>
                               )) || <p className="text-[11px] font-bold text-slate-900">1x {order?.title}</p>}
                            </div>
                         </div>
                            ...
                            <div className="space-y-3">
                               {STRATEGIES.map(s => (
                                  <button 
                                    ...
                                    className="w-full p-5 bg-white border border-slate-100 rounded-2xl text-left group hover:border-slate-900 hover:shadow-md transition-all flex items-center justify-between"
                                  >
                                     <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-[#94a3b8] group-hover:bg-slate-900 group-hover:text-white transition-all border border-slate-50"><s.icon size={18} /></div>
                                        <div className="space-y-0.5">
                         ...
                         <motion.div key="s2" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-8">
                            <div className="space-y-3">
                               ...
                               <textarea 
                                 ...
                                 required
                                 className="w-full h-32 bg-white border border-slate-100 rounded-xl p-5 text-[13px] font-medium text-slate-900 placeholder:text-slate-300 focus:border-slate-900 transition-all outline-none resize-none leading-relaxed"
                               />
                            </div>
