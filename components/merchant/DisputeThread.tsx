...
export default function DisputeThread({ dispute, onClose }: DisputeThreadProps) {
  ...
  const StrategyCard = ({ id, title, desc, icon: Icon, color }: any) => (
    <button 
      ...
      className="w-full p-6 bg-white border border-slate-100 rounded-2xl text-left group hover:border-slate-900 hover:shadow-md transition-all flex items-center justify-between"
    >
       <div className="flex gap-5 items-center">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center border border-slate-100 group-hover:bg-slate-900 group-hover:text-white transition-all`}>
             <Icon size={20} />
          </div>
       ...
       <ArrowRight size={16} className="text-slate-200 group-hover:text-slate-900 transition-all" />
    </button>
  );
       ...
       >
          ...
          <div className="px-8 py-8 border-b border-slate-50 flex items-center justify-between bg-white/80 backdrop-blur-xl z-20">
             <div className="flex items-center gap-5">
                <button onClick={step === 1 ? onClose : () => setStep(1)} className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-[#94a3b8] hover:text-slate-900 border border-slate-50 transition-all">
                   <ChevronLeft size={18} />
                </button>
                <div className="space-y-1">
                   <h2 className="text-[17px] font-bold tracking-tight text-slate-900">{step === 1 ? 'Select Strategy' : 'Execute Action'}</h2>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-10 no-scrollbar pb-32">
             ...
             <section className="space-y-6">
                <div className="bg-slate-50/50 border border-slate-100 rounded-2xl divide-y divide-slate-100 overflow-hidden">
                   ...
                   <div className="px-6 py-4 flex items-center justify-between text-[13px] font-bold">
                      ...
                      <span className="text-slate-900">RM {order?.total?.toFixed(2)}</span>
                   </div>
                </div>
             </section>
             ...
             <AnimatePresence mode="wait">
                {step === 1 ? (
                   ...
                   >
                      <div className="space-y-1">
                         <h3 className="text-[15px] font-bold text-slate-900">How would you like to handle this?</h3>
                   ...
                   >
                      <div className="space-y-1">
                         <h3 className="text-[15px] font-bold text-slate-900">Finalize Your {strategy} Strategy</h3>
                               ...
                               <div className="space-y-6">
                                  <div className="space-y-2">
                                     ...
                                     <textarea 
                                       ...
                                       className="w-full h-40 bg-white border border-slate-100 rounded-xl p-5 text-[14px] font-medium text-slate-900 focus:border-slate-900 transition-all outline-none resize-none leading-relaxed"
                                     />
                                  </div>
                                  <div className="space-y-2">
                            ...
                            {strategy === 'PARTIAL' && (
                               <div className="space-y-6">
                                  <div className="p-6 bg-slate-50/50 border border-slate-100 rounded-2xl space-y-4">
                                     <p className="text-[12px] font-bold text-slate-900">Set Compromise Amount</p>
